...
### 8-5. 스토리지 — 3노드+복제 상태에서 다시 재보니

같은 900만 행 기준으로 디스크 사용량도 다시 쟀다. 이번엔 Ignite가 3노드+`backups=1`(복제본 1개),
PostgreSQL은 단일 인스턴스+`device_id` 인덱스가 걸린 상태라 조건이 동일하지 않다는 점을 감안해야
한다.

| 항목 | PostgreSQL | Ignite (3노드 합산, WAL 제외) |
|---|---|---|
| 크기 | 1,178 MB (테이블 1,118MB + 인덱스 59MB) | 6,861 MB |
| 배수 | 기준 | 약 5.8배 |

Ignite가 5.8배 더 쓰는데, 이 중 절반(`backups=1`로 인한 2벌 저장)은 가용성을 얻으려고 의도적으로
지불한 비용이다 — PostgreSQL은 지금 복제본이 0개라 노드 하나 죽으면 서비스가 멈추지만, Ignite는
노드 하나가 죽어도 데이터가 남는다. 복제 비용을 제외해도(약 3.4GB) PostgreSQL 대비 약 2.9배인데,
이건 Ignite의 범용 바이너리 직렬화가 PostgreSQL의 고정 길이 튜플보다 원래 무겁기 때문으로
...
**처음엔 측정 가능한 이득이 없었다.** 1차에서는 직결이 근소하게 빨랐고 2차에서는 pooler가
근소하게 빨랐다 — 순위가 뒤집힐 정도로 차이가 run-to-run 편차(±6% 수준) 안에 묻힌다.
"매 연결마다 postgres 백엔드 프로세스를 새로 띄우는 비용을 서버 커넥션 재사용으로 없앤다"는
PgBouncer의 교과서적 존재 이유가 이 조건에서는 전혀 안 보였다 — 원인을 3단계로 파봤다.

### 9-3b. 원인 규명 1단계 — 인증 방식부터 확인

- `password_encryption = scram-sha-256`, `pg_authid` 확인 결과 `eds` 유저는 실제로
  `SCRAM-SHA-256$4096:...` (4096 iteration) — 여러 라운드트립 + PBKDF2라 원래 비싼 인증.
- 그런데 PgBouncer 파드의 실제 설정(`/controller/configs/pgbouncer.ini`, `auth_type=hba`)을
  까보니, **클라이언트↔PgBouncer 구간은 `pg_hba.conf`에서 `md5`를 쓰고 있었다** — SCRAM보다
  훨씬 싼 단일 라운드트립 해시 인증. Postgres 자체의 `pg_hba.conf`(직결 경로)는
  `host all all all scram-sha-256`.
- 즉 **pooler 쪽이 더 싼 인증을 쓰는데도 안 빨랐다** — 뭔가가 그 차이를 가리고 있다는 신호.

### 9-3c. 원인 규명 2단계 — TLS를 꺼보니 진짜 차이가 나왔다

기본 상태(`client_tls_sslmode=prefer` + Postgres `ssl=on`)에서는 두 경로 다 TLS 핸드셰이크를
탄다. `PGSSLMODE=disable`로 재측정(20 동시, `-C`, 10초):

| 대상 | tps | 평균 지연 | 연결 시간 |
|---|---|---|---|
| 직결 (SCRAM, TLS 없음) | 227.0 | 88.1ms | 17.6ms |
| pooler 경유 (md5, TLS 없음) | **268.3** | **74.5ms** | **14.9ms** |

**TLS를 끄니 pooler가 18% 더 빨랐다.** 기본 상태(TLS 켜짐)에서 차이가 안 보인 이유는 TLS
핸드셰이크(비대칭 암호화 연산) 비용이 md5 vs SCRAM 인증 비용 차이보다 훨씬 커서, 그 차이를
통째로 가리고 있었기 때문이다.

### 9-3d. 원인 규명 3단계 — 동시 접속 100으로 올리니 진짜 존재 이유가 드러났다

TLS는 다시 기본값(켜짐)으로 두고 동시 클라이언트를 100으로 올려서(`-C`, 15초) 재측정:

| 대상 | 결과 |
|---|---|
| 직결 (`eds-pg-rw`) | `FATAL: remaining connection slots are reserved for roles with the SUPERUSER attribute` — **일부 클라이언트 접속 실패**. `max_connections=100`인데 100개가 동시에 재연결을 시도하니 슬롯 고갈. tps=142.1(불완전 종료) |
| pooler 경유 (`eds-pg-pooler-rw`) | **에러 없이 전부 처리**. `default_pool_size=25`로 실제 백엔드 연결을 25개로 눌러두고 나머지 클라이언트는 대기시키는 구조라 Postgres 입장에서는 항상 25개 이하로만 보임. tps=146.5 |

tps 숫자 자체는 비슷하지만(142 vs 147, 워크로드 자체가 병목이라), **핵심은 "죽지 않고 처리한다"는
것** — 이게 PgBouncer의 진짜 가치였다.

### 9-3e. 결론

PgBouncer 효과가 처음에 안 보였던 건 두 가지가 겹친 결과였다:
1. TLS 핸드셰이크 비용이 인증 방식 차이(md5 vs SCRAM)를 가릴 만큼 컸다.
2. 처음 테스트한 동시성(20)이 `max_connections`(100)에 전혀 위협이 안 되는 수준이라, admission
   control이라는 PgBouncer의 핵심 가치가 발동할 상황 자체가 아니었다.

**"쿼리가 빨라지는가"가 아니라 "동시 접속이 `max_connections`를 넘어설 수 있는 상황에서 서비스가
죽지 않는가"로 PgBouncer를 평가해야 한다** — 9-3d의 100 동시 접속 테스트가 정확히 그 상황이다.
